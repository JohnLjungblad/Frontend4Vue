        ...
        <section class="watchlist">
          ...
          <div v-if="watchlist.length > 0">
            <li v-for="(item, index) in watchlist" :key="index" class="watchlist-list">
              {{ item.stockName.toUpperCase() }} - ${{ item.marketPrice }} <button
                @click="removeFromWatchlist(item.stockName)">❌</button>
            </li>
          </div>
        </section>
...
export default {
  ...
  methods: {
    updateWatchlistButton() {
      const stockAlreadyExists = this.watchlist.some(item => item.stockName === this.stockName);
    ...
    removeFromWatchlist(stockNameToRemove) {
      const updatedWatchlist = this.watchlist.filter(item => item.stockName !== stockNameToRemove);
      this.watchlist = updatedWatchlist;
      this.updateWatchlistButton();
    },

    addToWatchlist() {
      let listItem = { stockName: this.stockName, marketPrice: this.marketPrice };
